...
describe("<AddNewCourseForm/>", () => {
  ...
  it("data initialize correctly", () => {
    let courseDataMock = {
      assessment: expect.any(Boolean),
      link: expect.any(String)
    };
    expect(wrapper.vm.courseData).toEqual(
      expect.objectContaining(courseDataMock)
    );
  ...
  it("form should call api and receive a success message", async () => {
    const courseData = {
      data: "data"
    };
    addCourse.mockResolvedValue({ data: "Success" });
    ...
    expect(wrapper.vm.key).toEqual(1);
  });
  it("form should call api and receive a error message", async () => {
    addCourse.mockRejectedValue(new Error("Async error"));
  ...
  it("should change data", async () => {
    ...
    expect(wrapper.vm.courseData.assessment).toBe(false);

    await wrapper.vm.getCheckboxInput(event);

    expect(wrapper.vm.courseData.assessment).toBe(true);
  });
});
